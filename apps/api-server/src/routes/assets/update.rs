...
pub async fn move_trash_file_path(
    library: &Library,
    active: FilePathRequestPayload,
    target: Option<FilePathRequestPayload>,
) -> Result<(), rspc::Error> {
    ...
    let active_file_path_data = library
        ...
        .trash()
        .find_first(vec![
            trash::id::equals(active.id),
            trash::materialized_path::equals(active.materialized_path.clone()),
            trash::is_dir::equals(active.is_dir),
            trash::name::equals(active.name.clone()),
        ])
        .exec()
        .await
        .map_err(sql_error)?;
    ...
    if let Some(target) = target.as_ref() {
        ...
        let target_file_path_data = library
            ...
            .trash()
            .find_first(vec![
                trash::id::equals(target.id),
                trash::materialized_path::equals(target.materialized_path.clone()),
                trash::is_dir::equals(true),
                trash::name::equals(target.name.clone()),
            ])
            .exec()
            .await
            .map_err(sql_error)?;
    ...
    let duplicated_file_path_data = library
        ...
        .trash()
        .find_first(vec![
            trash::materialized_path::equals(new_materialized_path.clone()),
            trash::name::equals(active.name.clone()),
        ])
        .exec()
        .await
    ...
    library
        ...
        .trash()
        .update(
            trash::id::equals(active_file_path_data.id),
            vec![trash::materialized_path::set(
                new_materialized_path.clone(),
            )],
        )
        .exec()
