...
export type Procedures = {
    ...
    mutations: 
        ...
        { key: "assets.process_video_asset", input: string, result: null } | 
        { key: "assets.process_video_metadata", input: string, result: null } | 
        ...
        { key: "crr.apply", input: ApplyPayload, result: null } | 
        { key: "crr.pull", input: string, result: string } | 
        { key: "crr.pull_dir", input: PullDirPayload, result: string } | 
...
export type FileHandlerTask = { id: number; assetObjectId: string; taskType: string; exitCode: number | null; exitMessage: string | null; startsAt: string | null; endsAt: string | null; createdAt: string; updatedAt: string }

export type Auth = { id: string; name: string }

export type SharePayload = { fileIdList: string[]; peerId: string }
...
export type FilePathRenamePayload = { id: string; isDir: boolean; materializedPath: string; oldName: string; newName: string }

export type VideoSegmentExportPayload = { verboseFileName: string; outputDir: string; assetObjectId: string; millisecondsFrom: number; millisecondsTo: number }
...
export type FilePathDeletePayload = { id: string; materializedPath: string; name: string }
...
export type TaskRedoRequestPayload = { assetObjectId: string; preserveArtifacts: boolean }

export type Pagination = { pageSize: number; pageIndex: number }

export type PullDirPayload = { dir: string; dirFilePathId: string }

export type FilePath = { id: string; isDir: boolean; materializedPath: string; relativePath: string | null; name: string; description: string | null; assetObjectId: string | null; createdAt: string; updatedAt: string | null }

export type TaskListRequestFilter = { assetObjectId?: string | null; assetObjectIds?: string[] | null }
...
export type TaskCancelRequestPayload = { assetObjectId: string; taskTypes: string[] | null }
...
export type FilePathRequestPayload = { id: string; isDir: boolean; materializedPath: string; name: string }
...
export type AssetObject = { id: string; hash: string | null; size: number | null; mimeType: string | null; createdAt: string; updatedAt: string | null }
...
export type ApplyPayload = { relativePath: string; pullResult: string }
...
export type MediaData = { id: string; width: number | null; height: number | null; duration: number | null; bitRate: number | null; hasAudio: boolean | null; assetObjectId: string | null; createdAt: string; updatedAt: string | null }
