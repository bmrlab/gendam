...
pub mod decode {
    use crate::proto::SpaceTunnelIdentityErr;

    use super::{Error, Uuid};
    use futures::io::{AsyncRead, AsyncReadExt};

    async fn read_u64_le(stream: &mut (impl AsyncRead + Unpin)) -> Result<u64, futures::io::Error> {
        let mut buf = [0; 8];
        stream.read_exact(&mut buf).await?;
        Ok(u64::from_le_bytes(buf))
    }

    async fn read_u16_le(stream: &mut (impl AsyncRead + Unpin)) -> Result<u16, futures::io::Error> {
        let mut buf = [0; 2];
        stream.read_exact(&mut buf).await?;
        Ok(u16::from_le_bytes(buf))
    }

    async fn read_u32_le(stream: &mut (impl AsyncRead + Unpin)) -> Result<u32, futures::io::Error> {
        let mut buf = [0; 4];
        stream.read_exact(&mut buf).await?;
    ...
    pub async fn string(stream: &mut (impl AsyncRead + Unpin)) -> Result<String, Error> {
        let len = read_u16_le(stream).await?;
    ...
    pub async fn buf(stream: &mut (impl AsyncRead + Unpin)) -> Result<Vec<u8>, Error> {
        let len = read_u32_le(stream).await?;
