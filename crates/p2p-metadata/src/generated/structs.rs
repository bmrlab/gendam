    ...
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                ...
                Ok(58) => msg.name = Some(r.read_string(bytes)?.to_owned()),
                Ok(66) => msg.operating_system = Some(r.read_string(bytes)?.to_owned()),
                Ok(74) => msg.device_model = Some(r.read_string(bytes)?.to_owned()),
                Ok(82) => msg.version = Some(r.read_string(bytes)?.to_owned()),
...
impl MessageWrite for Identify {
    fn get_size(&self) -> usize {
        ...
        + self.operating_system.as_ref().map_or(0, |m| 1 + sizeof_len((m).len()))
        + self.device_model.as_ref().map_or(0, |m| 1 + sizeof_len((m).len()))
    ...
    fn write_message<W: WriterBackend>(&self, w: &mut Writer<W>) -> Result<()> {
        ...
        if let Some(ref s) = self.name { w.write_with_tag(58, |w| w.write_string(&**s))?; }
        if let Some(ref s) = self.operating_system { w.write_with_tag(66, |w| w.write_string(&**s))?; }
        if let Some(ref s) = self.device_model { w.write_with_tag(74, |w| w.write_string(&**s))?; }
        if let Some(ref s) = self.version { w.write_with_tag(82, |w| w.write_string(&**s))?; }
        Ok(())
    }
}
