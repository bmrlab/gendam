...
pub const PROTOCOL_NAME: StreamProtocol = StreamProtocol::new("/ipfs/metadata/1.0.0");

pub const PUSH_PROTOCOL_NAME: StreamProtocol = StreamProtocol::new("/ipfs/metadata/push/1.0.0");
...
impl TryFrom<proto::Identify> for Info {
    ...
    fn try_from(msg: proto::Identify) -> Result<Self, Self::Error> {
        ...
        let info = Info {
            ...
            name: msg.name,
            operating_system: msg.operating_system,
...
// Into Metadata
impl Into<HashMap<String, String>> for Info {
    fn into(self) -> HashMap<String, String> {
        let mut map = HashMap::new();
        self.name.map(|name| map.insert("name".to_string(), name));
        self.operating_system
            .map(|os| map.insert("operating_system".to_string(), os));
        self.device_model
            .map(|device_model| map.insert("device_model".to_string(), device_model));
        self.version
            .map(|version| map.insert("version".to_string(), version));
        map
    }
}
